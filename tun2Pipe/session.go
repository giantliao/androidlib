...
func newSession(ip4 *layers.IPv4, tcp *layers.TCP, srvPort int, bp bool) *Session {
	...
	log.Println(fmt.Sprintln("New Tcp Session:", s.ToString()))
	return s
}
...
func (dp *directPipe) readingIn() {
	...
	if _, err := io.Copy(dp.Left, dp.Right); err != nil {
		log.Println(fmt.Sprintln("Tun Proxy pipe right 2 left finished:", err))
		dp.expire()
	}
}
...
func (dp *directPipe) writeOut(buf []byte) error {
	if _, e := dp.Right.Write(buf); e != nil {
		log.Println(fmt.Sprintln("Tun Proxy pipe left 2 right err:", e))
		dp.expire()
		return e
	}
	return nil
