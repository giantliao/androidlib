...
func (t2s *Tun2Pipe) simpleForward(conn net.Conn) {
	...
	if s == nil {
		log.Println(fmt.Sprintln("Can't proxy this one:", leftConn.RemoteAddr()))
		return
	}
	...
	log.Println(fmt.Sprintln("Tun New conn for tcp session:", s.ToString()))
	...
	for {
		...
		if e != nil {
			if e != io.EOF {
				log.Println(fmt.Sprintln("Tun Read from left conn err:", e))
			}
			return
		}
		...
		if s.Pipe == nil {
			...
			if e != nil {
				log.Println(fmt.Sprintln("Dial remote err:", e))
				return
			}
			rightConn := c.(*net.TCPConn)
			rightConn.SetKeepAlive(true)
			log.Println(fmt.Sprintf("TCP:Tun Pipe dial success: %s->%s:", rightConn.LocalAddr(), s.ToString()))
