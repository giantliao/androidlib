...
func WrapIPPacketForUdp(srcIp, DstIp net.IP, srcPort, dstPort int, payload []byte) []byte {
	...
	if err := udp.SetNetworkLayerForChecksum(ip4); err != nil {
		log.Println(fmt.Sprintln("Udp Wrap ip packet check sum err:", err))
		return nil
	}
	...
	if err := gopacket.SerializeLayers(b, opt, ip4, udp, gopacket.Payload(payload)); err != nil {
		log.Println(fmt.Sprintln("Wrap dns to ip packet  err:", err))
		return nil
	}
...
func ProtectConn(conn syscall.Conn) error {
	...
	if err != nil {
		log.Println(fmt.Sprintln("Protect Err:", err))
		return err
	}
...
func ChangePacket(ip4 *layers.IPv4, tcp *layers.TCP) []byte {
	...
	if err := tcp.SetNetworkLayerForChecksum(ip4); err != nil {
		log.Println(fmt.Sprintln("set tcp layer check sum err:", err))
		return nil
	}
	...
	if err := gopacket.SerializeLayers(b, opt, ip4, tcp, gopacket.Payload(tcp.Payload)); err != nil {
		log.Println(fmt.Sprintln("Wrap Tcp to ip packet  err:", err))
		return nil
	}
...
func PrintFlow(pre string, ip4 *layers.IPv4, tcp *layers.TCP) {
	log.Println(fmt.Sprintf("%s	TCP <%d:%d> (%s:%d)->(%s:%d)",
		pre, tcp.Seq, tcp.Ack,
		ip4.SrcIP, tcp.SrcPort,
		ip4.DstIP, tcp.DstPort))

	if tcp.SYN {
		log.Println(fmt.Sprintln(" SYN"))
	}
	if tcp.ACK {
		log.Println(fmt.Sprintln(" ACK"))
	}
	if tcp.FIN {
		log.Println(fmt.Sprintln(" FIN"))
	}
	if tcp.PSH {
		log.Println(fmt.Sprintln(" PSH"))
	}
	if tcp.RST {
		log.Println(fmt.Sprintln(" RST"))
	}
	...
	case 'G', 'H', 'P', 'D', 'O', 'T', 'C':
		{
			...
			if r, _ := http.ReadRequest(reader); r != nil {
				log.Println(fmt.Sprintln("---===>>>Success host:", r.Host))
				return r.Host
			}
		}
	case 0x16:
...
func getSNI(data []byte) string {
	...
	log.Println(fmt.Sprintln("---===>>>Success SNI:", string(sni)))
	return string(sni)
}
